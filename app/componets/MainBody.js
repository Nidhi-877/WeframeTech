import React from "react";

import { LiaSearchSolid } from "react-icons/lia";
import { BsBellFill } from "react-icons/bs";
import { BsPersonVideo3 } from "react-icons/bs";
import { BsCheckCircle } from "react-icons/bs";
import { PiFolderNotchDuotone } from "react-icons/pi";
import { BiSolidDownArrow } from "react-icons/bi";
import Header from "./Header";
import Processbar from "./Processbar";

const MainBody = () => {
  return (
    <>
    
    <div className="navbar-main      ">
      <div className=" flex  items-center navbar    ">
        <div className="  relative flex items-center ml-3  search-bar  ">
          <LiaSearchSolid size={28} className=" absolute  left-4  text-center capitalize color-secondary "/>
          <input type="text" placeholder="Search here" className="search-box" />
        </div>
        <div className=" other-menu  ">
          <h3 className="pl-10 underline">OTHER MENUS</h3>
        </div>
        <div className="flex  nav-icons">
          <span >
            <BsBellFill className="color-secondary" />
            <p className="nav-badges">12</p>
          </span>
          <span>
            <BsPersonVideo3  className="inactive-contact"/>
            <p className="nav-badges">5</p>
          </span>
          <span>
            <BsCheckCircle  className="color-secondary" />
            <p className="nav-badges">2</p>
          </span>
          <span>
            <PiFolderNotchDuotone className="color-secondary" />
          <p className="nav-badge-pink">!</p>
          </span>
        </div>
        <div >
          <div className="flex items-center rounded-3xl px-5 p-2 mr-2 color-primary">
            <img className="w-4 h-4  rounded-xl mr-2"
              src="data:image/png;base64,"
              alt="not"
            />
            <h3 className="mr-2">ENGLISH</h3>
            <span className="mr-2 color-icon-primary">
              <BiSolidDownArrow />
            </span>
          </div>
        </div>
        <div className="flex items-center px-3 usermenu">
          <span className=" bg-slate-300  h-10 w-12 rounded-xl "></span>
          <div>
            <h4 className="px-3">Instructor Day</h4>
            <p className="px-4 text-sm color-secondary">Super Admin</p>
          </div>

          <span className="color-icon-primary">
            <BiSolidDownArrow />
          </span>
        </div>
      </div>
     
     <Header/>
     <Processbar/>
    </div>
    
    </>
    
  );
};

export default MainBody;
